...
vec4 voxelAmbientOcclusion(ivec4 modelData, vec3 pos, vec3 d1, vec3 d2) {
	bvec4 side = bvec4(bool(getSafeVoxelData(modelData, ivec3(pos + d1))),
		bool(getSafeVoxelData(modelData, ivec3(pos + d2))),
		bool(getSafeVoxelData(modelData, ivec3(pos - d1))),
		bool(getSafeVoxelData(modelData, ivec3(pos - d2))));
	bvec4 corner = bvec4(bool(getSafeVoxelData(modelData, ivec3(pos + d1 + d2))),
		bool(getSafeVoxelData(modelData, ivec3(pos - d1 + d2))),
		bool(getSafeVoxelData(modelData, ivec3(pos - d1 - d2))),
		bool(getSafeVoxelData(modelData, ivec3(pos + d1 - d2))));
...
HitData traceModel(Ray ray,
	...
	vec3 mapPos = floor(ray.origin + ray.direction * max(modelHitNear - 0.001, 0.0));
	...
	for(int i = 0; i < RENDER_DISTANCE; i++){
		...
		material = getVoxelData(modelData, ivec3(mapPos));
		if (material > 0) {
			...
			ambient = voxelAmbientOcclusion(modelData, mapPos - rayStep * mask, mask.zxy, mask.yzx);
...
float traceModelCheap(Ray ray, int modelIndex) {
...
float traceCheap(Ray ray) {
	int nModels = floatBitsToInt(texelFetch(u_modelData, 0).r);
	float result = WORLD_RENDER_DISTANCE;

	// Check for intersection with every model
	for(int i = 0; i < MAX_MODELS; ++i){
		if(i >= nModels) break;

		float newResult = traceModelCheap(ray, i);
